        ...
        <div className="min-h-screen p-4 md:p-8">
            <div className="container mx-auto max-w-6xl">
                <div className="flex justify-between items-center mb-6">
                    <button
                        className="btn btn-info px-7 text-white border-none hover:opacity-90 transition-all mt-4 rounded-4xl text-base"
                        onClick={() => navigate(-1)}
                    >
                        ← Back to Foods
                    </button>
                        ...
                        <div className="pt-4 m-2 bg-info/10 border-3 border-info/50 rounded-3xl p-4">
                            ...
                            <div className="flex items-center gap-4">
                                <img
                                    ...
                                    alt={food.user_name}
                                    className="w-20 h-20 rounded-full object-cover border-4 border-[#75805c]"
                                />
                                <div>
                                    <p className="font-black text-xl text-[#75805c]">{food.user_name}</p>
                                    <p className="text-info font-bold text-sm">{food.email}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="bg-base-200 border-5 border-base-300 rounded-3xl p-8 shadow-xl">
                        <h1 className="text-4xl font-black text-success mb-2">{food.food_name}</h1>
                        <p className="text-info font-bold mb-6">Shared by {food.user_name}</p>

                        <div className="space-y-4 mb-8">
                            <div className="flex items-center gap-3 p-3 bg-accent/20 rounded-xl">
                                <div className="w-12 h-12 text-3xl bg-base-200 rounded-full flex items-center justify-center border-2 border-base-300">
                                    <span>📦</span>
                                </div>
                                <div>
                                    <p className="text-sm font-bold">Quantity</p>
                                    <p className="font-black text-success">{food.food_quantity}</p>
                                </div>
                            </div>

                            <div className="flex items-center gap-3 p-3 bg-accent/20 rounded-xl">
                                <div className="w-12 h-12 text-2xl bg-base-200 rounded-full flex items-center justify-center border-2 border-base-300">
                                    <span>📍</span>
                                </div>
                                <div>
                                    <p className="text-sm font-bold">Pickup Location</p>
                                    <p className="font-black text-success">{food.pickup_location}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 p-3 bg-accent/20 rounded-xl">
                                ...
                                <div>
                                    ...
                                    <p className="font-black text-success">{new Date(food.expire_date).toLocaleDateString()}</p>
                                </div>
                            </div>
                        </div>
                        <div className="mt-8 bg-base-100 border-5 border-base-300 rounded-3xl p-8 shadow-xl">
                            <h3 className="text-xl font-black text-success mb-4">Additional Notes</h3>
                            <p className="text-[#75805c] font-semibold">
            ...
            <dialog id="food_requests_modal" className="modal">
                <div className="modal-box rounded-4xl w-11/12 max-w-7xl max-h-[80vh] overflow-hidden">
                    <h3 className="text-3xl font-black text-center mb-5 text-success">Food Requests</h3>
                    <div className="overflow-y-auto max-h-[60vh]">
                        <table className="table">
                            <thead className="bg-success text-white sticky top-0">
                                <tr>
                            ...
                            <tbody>
                                {foodRequests && foodRequests.length > 0 ? (
                                    foodRequests.map((request) => (
                                        <tr key={request._id} className="hover:bg-accent/10">
                                            <td>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="font-black text-[#575f43]">{request.name}</div>
                                                        <div className="text-sm text-info">{request.userEmail}</div>
                                                    </div>
                                                </div>
                                            </td>
                    ...
                    <div className="modal-action mt-4">
                        <form method="dialog">
                            <button className="btn btn-info rounded-full text-white">Close</button>
                        </form>
                    </div>
                </div>
            </dialog>
